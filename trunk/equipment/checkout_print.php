...
if (isset($HTTP_GET_VARS['KitID'])) {
  $colname_Recordset1 = (get_magic_quotes_gpc()) ? $HTTP_GET_VARS['KitID'] : addslashes($HTTP_GET_VARS['KitID']);
}
mysql_select_db($database_equip, $equip);
$query_Recordset1 = sprintf("SELECT * FROM kit WHERE ID = %s", $colname_Recordset1);
$Recordset1 = mysql_query($query_Recordset1, $equip) or die(mysql_error());
$row_Recordset1 = mysql_fetch_assoc($Recordset1);
$totalRows_Recordset1 = mysql_num_rows($Recordset1);

$ServerKitID = $HTTP_GET_VARS['KitID'];
$colname_Recordset2 = "1";
if (isset($HTTP_GET_VARS['KitID'])) {
  $colname_Recordset2 = (get_magic_quotes_gpc()) ? $HTTP_GET_VARS['KitID'] : addslashes($HTTP_GET_VARS['KitID']);
}
mysql_select_db($database_equip, $equip);
$query_Recordset2 = sprintf("SELECT * FROM kit_accessorytype INNER JOIN accessorytype ON accessorytype.ID = kit_accessorytype.ID WHERE KitID = $ServerKitID;");
...
check
off.)
<form action="checkoutaction.php" method="get">
<input type="hidden" name="KitID" value="<? echo $HTTP_GET_VARS['KitID'] ?>">
